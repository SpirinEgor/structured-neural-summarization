...
from parsers.naturallanguage.graphtextrepr import DependencyEdge, GraphTextRepresentation, Token
...
def parse_sample(
    datapoint, provenance: str, headline_getter: Optional[Callable[[Any], List[str]]] = None
    ...
    for sentence_idx, sentence in enumerate(all_sentences):
        ...
        for token_idx, token in enumerate(sentence_tokens):
            ...
            if token_idx + 1 < len(sentence_tokens) - 1 and sentence_tokens[token_idx + 1]["NER"] != token["NER"]:
                ...
                graph_text_representation.add_entity(
                    token["NER"], sentence_idx, before_start_token_idx + 1, token_idx + 1
                )
    ...
    for sentence_idx, sentence in enumerate(all_sentences):
        if (
            "collapsed-dependencies" not in sentence or sentence["collapsed-dependencies"] is None
        ) and get_collapsed_dependencies(sentence) is None:
            continue
        ...
        for dependency in collapsed_deps["dep"]:
            ...
            graph_text_representation.add_dependency_edge(
                DependencyEdge(
    ...
    if coreferences is not None:
        ...
        for coreference in coreferences["coreference"]:
            ...
            for mention in all_mentions:
                if mention.get("@representative") == "true" or (
                    mention["sentence"] == representative["sentence"] and mention["head"] == representative["head"]
                ):
                    continue
                graph_text_representation.add_coreference(
                    int(mention["sentence"]) - 1,
                    int(mention["head"]) - 1,
                    int(representative["sentence"]) - 1,
                    int(representative["head"]) - 1,
                )

    return TextSummary(summary_sentence=headline_tokens, main_text=graph_text_representation)
    ...
    with gzip.GzipFile(args["OUT_FILEPATH_PREFIX"] + "-inputs.jsonl.gz", "wb") as text_input_file:
        with gzip.GzipFile(args["OUT_FILEPATH_PREFIX"] + "-summaries.jsonl.gz", "wb") as summaries_file:
            ...
            for file_idx, file in enumerate(glob.glob(args["INPUT_XML_ARTICLES_PATTERN"])):
                if not file.endswith(".article.xml"):
                    continue
