...
def convert_graph(graph_data: Dict[str, Any], add_eq_edges: bool) -> Dict[str, Any]:
    ...
    new_graph["edges"] = [
        (e[0], old_to_new_id[e[1]], old_to_new_id[e[2]])
...
def run(arguments):
    save_jsonl_gz(
        arguments["OUTPUT_TARGET_JSONL_GZ"],
        data=(
            convert_graph(g, arguments["--add-eq-edges"]) for g in iteratate_jsonl_gz(arguments["INPUT_GRAPH_JSONL_GZ"])
        ),
    )
