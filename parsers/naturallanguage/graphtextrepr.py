...
Token = NamedTuple("Token", [("word", str), ("lemma", str), ("pos_tag", str)])

DependencyEdge = NamedTuple(
    "DependencyEdge", [("dependency_type", str), ("sentence_idx", int), ("from_idx", int), ("to_idx", int)]
)


Entity = NamedTuple("Entity", [("entity_type", str), ("sentence_idx", int), ("start_idx", int), ("end_idx", int)])
...
class GraphTextRepresentation:
    ...
    def add_entity(self, entity_type: str, sentence_idx: int, from_idx: int, to_idx: int) -> None:
        ...
        self.__entities[sentence_idx].add(Entity(entity_type, sentence_idx, from_idx, to_idx))

    def __entities_covered_by(self, sentence_idx: int, location_idx: int) -> Optional[Entity]:
        entities = [
            entity for entity in self.__entities[sentence_idx] if entity.start_idx <= location_idx < entity.end_idx
        ]
    ...
    def add_coreference(
        self, from_sentence_idx: int, from_head_idx: int, to_sentence_idx: int, to_head_idx: int
    ) -> None:
        """
        ...
        self.__coreferences.append(
            (
                get_coreference_location(from_sentence_idx, from_head_idx),
                get_coreference_location(to_sentence_idx, to_head_idx),
            )
        )
    ...
    def to_graph_object(self):
        ...
        for dep_edge in self.__dependency_edges:
            node_edges.append(
                (
                    dep_edge.dependency_type,
                    word_node_location[dep_edge.sentence_idx][dep_edge.from_idx],
        ...
        return {
            ...
            "edges": node_edges,
            "backbone_sequence": list(range(num_words)),
        ...
        with StringIO() as sb:
            ...
            for sentence_idx, entities in self.__entities.items():
                for entity in entities:
                    sb.write(
                        'At sentence %s, entity %s with text "%s"\n'
                        % (
                            sentence_idx,
                            entity.entity_type,
                            " ".join(
                                t.word for t in self.__tokenized_text[sentence_idx][entity.start_idx : entity.end_idx]
                            ),
                        )
                    )
            ...
            def mention_text(mention_location: Union[Tuple, Entity]) -> str:
                if type(mention_location) is Entity:
                    ...
                    return '%s("%s")@Sentence %s' % (
                        mention_location.entity_type,
                        " ".join(
                            t.word
                            for t in self.__tokenized_text[sent_idx][
                                mention_location.start_idx : mention_location.end_idx
                            ]
                        ),
                        sent_idx,
                    )
                    ...
                    return '"%s"@Sentence %s' % (
                        self.__tokenized_text[mention_location[0]][mention_location[1]].word,
                        mention_location[0],
                    )
